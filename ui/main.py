...
def build_ui(root: tk.Tk, runtime: Runtime) -> None:
    ...
    def on_trainer_settings_changed(settings: dict) -> None:
        ...
        save_config(config)
        runtime.notify_profile_updated(settings)
    ...
    # Restore shared settings from config, if any.
    settings_conf = config.get("settings") or {}
    legacy_pet = config.get("pet") or {}
    if legacy_pet and not any(settings_conf.get(key) for key in legacy_pet.keys()):
        settings_conf.update(legacy_pet)
        config["settings"] = dict(settings_conf)
        save_config(config)
    if settings_conf:
        pet_tab.apply_settings(settings_conf)
    # Runtime orchestration now lives alongside session joins.

    def _start_trainer_runtime() -> None:
        shared_settings = pet_tab.collect_settings()
        input_device = pet_tab.input_device
        runtime.start_runtime("trainer", shared_settings, input_device)

    def _start_pet_runtime() -> None:
        shared_settings = pet_tab.collect_settings()
        input_device = pet_tab.input_device
        runtime.start_runtime("pet", shared_settings, input_device)
    ...
    def runtime_status_provider(role: str | None) -> dict[str, str]:
        ...
        osc_text = format_osc_status(role, osc_status)

        status = {
            "osc": osc_text,
            "osc_details": osc_text,
            "pishock": format_pishock_status(pishock_status),
            "whisper": whisper_status,
        }
        username = runtime.get_server_username()
        if username:
            status["username"] = username

        runtime.publish_runtime_status(role, status)
        return status

    def on_pet_profile_selected(pet_client_id: str, profile_name: str | None) -> None:
        if not profile_name:
            runtime.assign_profile_to_pet(pet_client_id, None, None)
            return
    ...
    if stored_username:
        ...
        runtime.set_server_username(stored_username)
    ...
    def _on_server_username_changed(*_) -> None:
        ...
        save_config(config)
        runtime.set_server_username(username or None)
    ...
    connection_status = ConnectionStatusPanel(main_frame, runtime)
    connection_status.grid(row=1, column=0, sticky="ew", padx=10, pady=(6, 4))

    event_log = EventLogPanel(main_frame, runtime)
    event_log.grid(row=2, column=0, sticky="nsew", padx=10, pady=(0, 4))

    whisper_log = WhisperLogPanel(main_frame, runtime)
